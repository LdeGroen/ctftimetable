...
const translations = {
  nl: {
    common: {
      ...
      // WIJZIGING: Tekst aangepast
      noDataFound: 'Geen voorstellingen gevonden voor %s.',
      // NIEUW: Specifieke tekst voor als filters geen resultaat opleveren
...
const StickyHeader = ({ isVisible, uniqueEvents, handleEventClick, handleFavoritesClick, handleFriendsFavoritesClick, hasFriendsFavorites, selectedEvent, currentView, language, handleLanguageChange, translations, onLogoClick, onReadPage }) => {
...
const EventNavigation = ({ onEventSelect, onFavoritesSelect, onFriendsFavoritesSelect, hasFriendsFavorites, uniqueEvents, language, translations }) => (
...
const TimetableDisplay = React.forwardRef(({
  ...
  // NIEUW: Filters doorgegeven voor de 'geen resultaten' boodschap
  ...
  if (displayedData.length === 0) {
    // WIJZIGING: Verbeterde logica voor de "geen resultaten" boodschap
...
const AppContent = () => {
  ...
  // WIJZIGING: Logica voor het filteren van datums op basis van actieve filters
  const datesForCurrentSelectedEvent = useMemo(() => {
    ...
    // Pas filters toe als er filters actief zijn
